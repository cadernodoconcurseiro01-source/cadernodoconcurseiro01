...
import { Subject, Contest } from '@/types/database';
...
interface AddDailyQuestionsDialogProps {
  ...
  subjects: Subject[];
  contests?: Contest[];
  contestSubjectMappings?: { contest_id: string; subject_id: string }[];
}

export function AddDailyQuestionsDialog({ onAdd, subjects, contests = [], contestSubjectMappings = [] }: AddDailyQuestionsDialogProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          {contests.length > 0 && (
            <div className="space-y-2">
              ...
              <Select value={selectedContestId || NO_CONTEST_VALUE} onValueChange={(v) => {
                setSelectedContestId(v === NO_CONTEST_VALUE ? '' : v);
                setSubjectId('');
              }}>
                <SelectTrigger>
